...
// `colours`: the map's paper (for the outline) and ink.
export async function makeSprites(ink: InkSet, roles: string[], mapPx: number, screenPx: number, done: Map<string, Sprite>, colours = { paper: "#fff", ink: "#1a1714" }): Promise<Map<string, Sprite>> {
  const px = mapPx / 1600;
  ...
  for (const role of roles) {
    ...
    for (const icon of ink[role] ?? []) {
      ...
      jobs.push(
        drawSprite(icon, typical, screenPerMap, px, colours)
...
async function drawSprite(icon: InkSymbol, typicalWidth: number, screenPerMap: number, px: number, colours: { paper: string; ink: string }): Promise<Sprite> {
  ...
  const svg =
    ...
    `<g color="${colours.paper}" stroke="${colours.paper}" stroke-width="${halo.toFixed(2)}" stroke-linejoin="round">${icon.body}</g>` +
    `<g color="${colours.ink}" stroke="none">${icon.body}</g></svg>`;
